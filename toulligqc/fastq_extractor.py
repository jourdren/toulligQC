...
class fastqExtractor:
    ...
    def init(self):
        ...
        if "barcode_arrangement" in self.dataframe_1d.columns:
            self.dataframe_1d["barcode_arrangement"] = self.dataframe_1d[
                "barcode_arrangement"
    ...
    def graph_generation(self, result_dict):
        ...
        add_image_to_result(
            ...
            images,
            time.time(),
            pgg.read_count_histogram(result_dict, self.images_directory),
        )
        add_image_to_result(
            self.quiet,
            images,
        ...
        if self.rich:
            add_image_to_result(
                ...
                images,
                time.time(),
                pgg.yield_plot(self.dataframe_1d, self.images_directory),
            )
        add_image_to_result(
            self.quiet,
            images,
            time.time(),
            pgg.read_quality_multiboxplot(self.dataframe_dict, self.images_directory),
        )
        add_image_to_result(
            self.quiet,
            images,
            time.time(),
            pgg.allphred_score_frequency(self.dataframe_dict, self.images_directory),
        )
        ...
        if self.rich:
            add_image_to_result(
                ...
                images,
                time.time(),
                pgg.plot_performance(self.dataframe_1d, self.images_directory),
            )
        add_image_to_result(
            self.quiet,
            images,
            time.time(),
            pgg.twod_density(self.dataframe_dict, self.images_directory),
        )
        ...
        if self.rich:
            add_image_to_result(
                ...
                images,
                time.time(),
                pgg.sequence_length_over_time(
                    self.dataframe_dict, self.images_directory
                ),
            )
            add_image_to_result(
                self.quiet,
                images,
                time.time(),
                pgg.phred_score_over_time(
                    self.dataframe_dict, result_dict, self.images_directory
                ),
            )
            ...
            if self.is_barcode:
                ...
                add_image_to_result(
                    ...
                    images,
                    time.time(),
                    pgg.barcode_percentage_pie_chart_pass(
                        self.dataframe_dict,
                ...
                if not (len(read_fail) == 1 and read_fail["other barcodes"] == 0):
                    add_image_to_result(
                        ...
                        images,
                        time.time(),
                        pgg.barcode_percentage_pie_chart_fail(
                            self.dataframe_dict,
                            self.barcode_selection,
                            self.images_directory,
                            barcode_alias,
                ...
                add_image_to_result(
                    ...
                    images,
                    time.time(),
                    pgg.barcoded_phred_score_frequency(
                        self.dataframe_dict, self.images_directory, barcode_alias
                    ),
                )
        return images
    ...
    def extract(self, result_dict):
        ...
        if self.rich:
            set_result_value(
                self, result_dict, "run.time", max(self.dataframe_1d["start_time"])
            )
        ...
        qscore_statistics = self.dataframe_1d["mean_qscore"].describe().drop("count")
    ...
    def _load_fastq_data(self):
        ...
        fq_df["passes_filtering"] = fq_df["passes_filtering"].astype(
            np.bool_ if is_numpy_1_24 else np.bool
        )
        ...
        if self.rich:
            ...
            if self.is_barcode:
                fq_df["barcode_arrangement"] = fq_df["barcode_arrangement"].astype(
                    "category"
                )
        ...
        if self.rich:
            for read in read_batch:
                ...
                if self.is_barcode:
                    ...
                    fastq_lines.append(
                        (len(read[1]), qscore, passes_filtering, start_time, ch)
                    )
        else:
            for read in read_batch:
                if len(read) > 0:
